#!/usr/bin/env python
""" stresstest.py:  A stres-tester for ConcurrentRotatingFileHandler

This utility spawns a bunch of processes that all try to concurently write to
...
complared to see if any log messages have been lost.
...
__version__ = '$Id: stresstest.py 5481 2008-05-21 15:18:22Z lowell $'
...
from fsa.releasetools.thirdparty.cloghandler import ConcurrentRotatingFileHandler
...
class RotateLogStressTester:
    ...
    def start(self):
        ...
        # If you suspect that the diff stuff isn't working, uncomment the next
        # line.  You should see this showup once per-process.
    ...
    def run(self):
        ...
        while c <= self.writeLoops:
            ...
            if self.random_sleep_mode and c % 1000 == 0:
                ...
                sleep(s)
            # break
        self.log.info("Done writting random log messages.")
...
parser = OptionParser(usage="usage:  %prog",
                      version=__version__,
                      description="Stress test the safelogrotate module.")
...
def main_client(args):
    ...
    if os.path.isfile(client):
        sys.stderr.write("Already a client using ouptut file %s\n" % client)
        sys.exit(1)
...
def main_runner(args):
    ...
    parser.add_option("--delay", metavar="secs",
                      ...
                      help="Wait SECS before spawing next processes.  "
                      "Default: %default")
    ...
    if not manager.checkExitCodes():
        sys.stderr.write("One or more of the child proceses has failed.\n"
                         "Aborting test.\n")
        sys.exit(2)
    ...
    print "Running interal diff:  (If the next line is 'end of diff', then the stress test passed!)"
